...
class SecurityController extends AbstractController
{
    ...
    public function forgottenPassword(Request $request, UsersRepository $usersRepository, TokenGeneratorInterface $tokenGenerator, EntityManagerInterface $entityManager, SendMailService $mail): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($user) {
                // On génère un token de reset
                $token = $tokenGenerator->generateToken();
                $user->setResetToken($token);
                $entityManager->persist($user);
                $entityManager->flush();

                // On génère un lien de reset du mot de passe
                ...
                // Envoi du mail
                $mail->send("no-reply@e-commerce.fr", $user->getEmail(), "Réinitialisation de mot de passe", "password_reset", $context);
